...
import static org.apache.iotdb.db.queryengine.plan.table.analyzer.ExpressionTreeUtils.asQualifiedName;
import static org.apache.iotdb.db.queryengine.plan.table.analyzer.Scope.BasisType.FIELD;
import static org.apache.iotdb.db.queryengine.plan.table.analyzer.Scope.BasisType.TABLE;
...
public class Scope {
  ...
  public Scope getQueryBoundaryScope() {
    Scope scope = this;
    Optional<Scope> localParent = scope.getLocalParent();
    while (localParent.isPresent()) {
      scope = localParent.get();
      localParent = scope.getLocalParent();
    }
    return scope;
  }
  ...
  public int getLocalScopeFieldCount() {
    int localParent = getLocalParent().map(Scope::getLocalScopeFieldCount).orElse(0);

    return localParent + getRelationType().getAllFieldCount();
  }
  ...
  /**
   * Starting from this, finds the closest scope which satisfies given predicate, within the query
   * boundary.
   */
  ...
  /**
   * This method analyzes asterisked identifier chain in 'all fields reference', according to SQL
   * standard. The following rules are applied: 1. if the identifier chain's length is lower than 4,
   * it is attempted to resolve it as a table reference, either in the local scope or in outer
   * scope. 2. if the identifier chain's length is greater than 1, it is attempted to match the
   * chain's prefix of length 2 to a field of type Row, either in the local scope or in outer scope.
   * Any available match in the innermost scope is picked and the 'identifier chain basis' is
   * defined. If more than one basis is defined, it results in a failure (ambiguous identifier
   * chain).
   *
   * <p>NOTE: only ambiguity between table reference and field reference is currently detected. If
   * two or more tables in the same scope match the identifier chain, they are indistinguishable in
   * current Scope implementation, and such query results in returning fields from all of those
   * tables. If two or more fields in the same scope match the identifier chain, such query will
   * fail later during analysis.
   *
   * <p>NOTE: Although references to outer scope tables are found, referencing all fields from outer
   * scope tables is not yet supported.
   */
  public Optional<AsteriskedIdentifierChainBasis> resolveAsteriskedIdentifierChainBasis(
      QualifiedName identifierChain, AllColumns selectItem) {
    ...
    if (length <= 3) {
      scopeForTableReference =
          findLocally(
              scope ->
                  scope.getRelationType().getAllFields().stream()
                      .anyMatch(field -> field.matchesPrefix(Optional.of(identifierChain))));
    }

    if (length >= 2) {
      scopeForFieldReference =
          findLocally(
              scope ->
                  scope.getRelationType().getAllFields().stream()
                      .anyMatch(
                          field ->
                              field.matchesPrefix(
                                      Optional.of(
                                          QualifiedName.of(identifierChain.getParts().get(0))))
                                  && field.getName().isPresent()
                                  && field
                                      .getName()
                                      .get()
                                      .equals(identifierChain.getParts().get(1))));
    }
    ...
    if (scopeForTableReference.isPresent()) {
      return Optional.of(
          new AsteriskedIdentifierChainBasis(
              TABLE,
              scopeForTableReference,
    ...
    if (scopeForFieldReference.isPresent()) {
      return Optional.of(
          new AsteriskedIdentifierChainBasis(FIELD, Optional.empty(), Optional.empty()));
    }

    return getOuterQueryParent()
        .flatMap(
            parent -> parent.resolveAsteriskedIdentifierChainBasis(identifierChain, selectItem));
  }
  ...
  public String toString() {
    return toStringHelper(this).addValue(relationId).toString();
  }
