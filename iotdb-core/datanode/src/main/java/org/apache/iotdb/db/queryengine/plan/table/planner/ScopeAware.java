...
import org.apache.iotdb.db.queryengine.plan.table.analyzer.CanonicalizationAware;
import org.apache.iotdb.db.queryengine.plan.table.analyzer.ResolvedField;
...
import static org.apache.iotdb.db.queryengine.plan.table.sql.util.AstUtil.treeEqual;
import static org.apache.iotdb.db.queryengine.plan.table.sql.util.AstUtil.treeHash;

/**
 * A wrapper for Expressions that can be used as a key in maps and sets.
 *
 * <p>Expressions are considered equal if they are structurally equal and column references refer to
 ...
 *
 * <p>"t.a" and "a" are considered equal because they reference the same field of "t"
 *
 * <p>Limitation: the expressions in the following query are currently not considered equal to each
 * other, even though they refer to the same field from the same table or named query "t". This is
 * because we currently don't assign identity to table references. (TODO: implement this)
 *
...
public class ScopeAware<T extends Node> {
  ...
  public static <T extends Node> ScopeAware<T> scopeAwareKey(
      T node, Analysis analysis, Scope scope) {
   ...
   *
   * <p>When both compared Nodes are column references, the following rules apply:
   *
   * <p>1. If both columns belong to scopes associated with subqueries of the current query, they
   * are compared by syntax (that is, textually, respecting the canonicalization rules).
   *
   * <p>2. If none of the columns belongs to some subquery scope, that is, either they belong to the
   * current query scope, or some outer scope, they are compared by resolved field.
   *
   * <p>3. If only one of the columns belongs to some subquery scope, they are not equal.
   *
   * <p>Note: it'd appear that hash() and equal() are inconsistent with each other in the case that:
   * - left.hasOuterParent(...) == true and right.hasOuterParent(...) == false -
   * leftField.getFieldId().equals(rightField.getFieldId()) == true Both fields would seem to have
   * different hashes but be equal to each other. However, this cannot happen because we *require*
   * that both expressions being compared be rooted in the same "query scope" (i.e., sub-scopes that
   * are local to each other) -- see ScopeAwareKey.equals(). If both fields have the same field id,
   * by definition they will produce the same result for hasOuterParent().
   */
  private Boolean scopeAwareComparison(Node left, Node right) {
    if (left instanceof Expression && right instanceof Expression) {
      ...
      if (analysis.isColumnReference(leftExpression)
          && analysis.isColumnReference(rightExpression)) {
        ...
        if (leftFieldInSubqueryScope && rightFieldInSubqueryScope) {
          return treeEqual(
              leftExpression,
              rightExpression,
              CanonicalizationAware::canonicalizationAwareComparison);
        }
      ...
      if (analysis.isColumnReference(expression)) {
        ...
        if (resolvedScope.hasOuterParent(queryScope)) {
          return OptionalInt.of(
              treeHash(expression, CanonicalizationAware::canonicalizationAwareHash));
        }
